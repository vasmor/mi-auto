...
4. Если `vendor/` не включён в архив — запустить `composer install` в директории темы
5. Перейти по адресу `/wp-admin/?miauto_setup=1` для импорта демо-контента
6. Проверить: Настройки → Чтение → Статическая главная страница
7. Проверить: Внешний вид → Меню → Привязки к областям
8. Проверить: Настройки → Постоянные ссылки (должно быть `/%postname%/`)
9. Проверить все страницы на фронтенде
